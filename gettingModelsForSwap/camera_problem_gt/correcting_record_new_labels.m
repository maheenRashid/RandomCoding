clear; close all;clc;
load('bad_names');
load('record_new_labels');

group_dir='/home/maheenr/AWS-DATA/data/shared_payload/annotated_models/skp_groupings';
cat_dir='/home/maheenr/AWS-DATA/data/shared_payload/annotated_models/skp_category';
...
for bad_no=1:numel(bad_names)
    ...
    record_bad_names(bad_no).cat_no_aft=-1*ones(size(cell_data{1}));
    record_bad_names(bad_no).cat_no_bef=cell_data{2};
    record_bad_names(bad_no).group_ids=cell_data{1};
end

record_new_labels=[record_new_labels record_bad_names];
save('record_new_labels_bad_names_in.mat','record_new_labels');
