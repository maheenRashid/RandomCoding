...
out_parent='swapModelInBox_cellCommands';
...
for name_no=1:numel(names)
    ...
    load(fullfile(['swapModel_' names{name_no} '_html.mat']),'cell_for_command');
    
    out_name=regexpi(names{name_no},'_','split');
    out_name=sprintf('_%s',out_name{end-1:end});
    out_name=['swapModelInBox' out_name];
