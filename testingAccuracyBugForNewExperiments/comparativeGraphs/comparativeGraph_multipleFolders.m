...
for i=1:numel(folders)
    ...
    idx=~cellfun(@isempty,names_and_scores(2,:));
    names_and_scores_all{1,i+1}=names_and_scores(1,idx);
    names_and_scores_all{2,i+1}=names_and_scores(2,idx);
...
scores_mine_auto=names_and_scores_all{2,3};
...
for i=1:size(diffs_sorted,2)
    [~,~,diff_sorted_curr]=getGraphsDiff(names_and_scores_all{2,i},str_graphs);
