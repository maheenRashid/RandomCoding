function [success,list_size,bad_list]=writingListFilesAfterReadingOverlap_hack(im_name,in_dir,out_dir,thresh,timeout,blacklist,boxes_done)
    success=1;
    bad_list=zeros(1,0);
    if nargin<4
        thresh=0.01;
    ...
    idx_done=find(boxes_done>0);
    for i=1:numel(idx_done)
        i_curr=idx_done(i);
        overlap(i_curr,idx_done)=1;
        overlap(idx_done,i_curr)=1;
    end
