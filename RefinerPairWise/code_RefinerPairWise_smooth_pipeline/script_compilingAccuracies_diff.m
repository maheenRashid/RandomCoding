...
for k_p=k_p_vec    
    ...
    names={mat_lists(:).name};
    temp=cell(size(names));
record_test=struct('name',names,'bin_rel',temp);
...
for i=1:numel(mat_lists)
    temp=load(fullfile(dir_accu_curr,mat_lists(i).name));
    bin_rel=temp.record_accuracy.pred;
    bin_rel=bin_rel(bin_rel(:,end)>0,:);
    record_test(i).bin_rel=bin_rel;
%     keyboard;
...
% 
    out_file_name=fullfile(out_dir,['test_merged.mat']);
save(out_file_name,'record_test');
