...
%         matchToGt=dpm_cat_no(i)==gt_cat_varsha
        matchToGt=find(gt_cat_varsha~=0);
            ...
            for dpm_box_no=1:size(dpm_boxes_curr,1)
                ...
                for gt_box_no=1:numel(matchToGt)
                    ...
                    if show>0
                        ...
                        fprintf('gt_label (blue): %d\n', gt_cat_varsha(matchToGt(gt_box_no)));
                        fprintf('dpm_label (red): %d\n', dpm_cat_no(i));
                        fprintf('bin: %d\n', bin);
                        keyboard;
                    else
                ...
                if sum(gt_cat_varsha(matchToGt(gt_box_no))== catsWeConsider)==0
                    gt_no
                    keyboard;
                end
                
                if bin==0 || sum(gt_cat_varsha(matchToGt(gt_box_no))== catsWeConsider)==0
                    bin_detect_curr(dpm_box_no)=0;
                else
                    bin_detect_curr(dpm_box_no)=gt_cat_varsha(matchToGt(gt_box_no));
                end
            end
        end
...
% save('dpm_greater_-1_bbox_record_withDetections_multiCat.mat')
