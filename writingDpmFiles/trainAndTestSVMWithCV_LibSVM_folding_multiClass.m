...
% dpm_svm_data(2,:)=fixClassLabels(dpm_svm_data(2,:),catsWeConsider);
...
for fold_no=1:5
    ...
    for f_combo_no=1:numel(feature_range_cell)
        ...
        %scale features and transform to sparse matrices
        ...
        %do crossValidation
        tic()
        [C,sigma,cp_best,cp_all]=getSVMArgs_LibSVM_MultiCat(features_train_sparse,class_train,k,c_pow_range,sigma_pow_range,weightsAndLabs);
        toc()
        ...
        record_svm{1,fold_no,f_combo_no}=bsr;
        record_svm{2,fold_no,f_combo_no}=C;
        record_svm{3,fold_no,f_combo_no}=sigma;
        record_svm{4,fold_no,f_combo_no}=cp_all;
%         keyboard
    end
end
